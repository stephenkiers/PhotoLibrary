...
I(
    "M0-01",
    "Blocking questions Q1-Q7 answered 2026-09-04 (Shared Library scope, edited-render handling, NAS volume1 assumption, Mar-2025 backup treated as full source, dedup definition, proxy targets, staging/compute host)",
    "M0",
    [],
    ["decision", "answered"],
    "S",
...
I(
    "M0-02",
    "Rotate leaked Immich API key and DB password; scrub old repos",
    "M0",
    [],
    ["security"],
    "S",
    ...
    [
        "New key issued in Immich, old key revoked",
        "No secret strings remain in any tracked file (gitleaks passes)",
    ],
)
I(
    "M0-03",
    "Procure/attach staging storage on the Intel i9 Mac and second/third backup drives",
    "M0",
    ["M0-01"],
    ["hardware"],
    "S",
    ...
    "bucket).",
    [
        "4TB staging disk mounted on the Intel Mac and benchmarked (>500 MB/s)",
        "Two backup targets identified and reachable",
    ],
)
I(
    "M0-04",
    "Bootstrap repo: uv project, typer CLI skeleton, ruff, mypy, pytest, pre-commit",
    "M0",
    [],
    ["infra"],
    "S",
    "",
    [
        "`uv run vault --help` lists all planned subcommands as stubs",
        "CI runs lint, type check, tests on push",
    ],
)
I(
    "M0-05",
    "Config schema and loader (single vault.toml + env overrides)",
    "M0",
    ["M0-04"],
    ["core"],
    "S",
    ...
    [
        ...
        "Example config committed as vault.example.toml",
    ],
)
I(
    "M0-06",
    "Hashing module: BLAKE3 full hash, xxh3 prehash, chunked verify",
    "M0",
    ["M0-04"],
    ["core"],
    "S",
    "",
    [
        "Hash 1 GB file in <2 s on the Intel i9 Mac",
        "Property test: hash(bytes) stable, verify() detects single-bit flip",
    ],
)
I(
    "M0-07",
    "Content-addressed store: put (temp+rename), get path, exists, verify, iterate",
    "M0",
    ["M0-06", "M0-05", "M0-14"],
    ["core", "data-safety"],
    "M",
    ...
    [
        "put is idempotent and atomic (kill -9 mid-copy leaves no partial object)",
        "verify(hash) rehashes and reports mismatch",
        "Works on SMB and local APFS/ext4",
    ],
)
I(
    "M0-08",
    "Append-only journal (jsonl, hash-chained, typed events)",
    "M0",
    ["M0-05"],
    ["core"],
    "M",
...
I(
    "M0-09",
    "Catalog: SQLite schema, migrations, rebuild-from-journal",
    "M0",
    ["M0-08", "M0-07"],
    ["core"],
    "L",
    ...
    [
        "rebuild on a fixture vault equals the incrementally built catalog (byte-for-byte dump)",
        "Migrations are forward-only and tested",
    ],
)
I(
    "M0-10",
    "Test fixture vault: ~200 synthetic and real assets covering every edge case",
    "M0",
    ["M0-04"],
    ["testing"],
    "M",
...
I(
    "M0-11",
    "Docker image for NAS (linux/amd64) with exiftool, vips, ffmpeg",
    "M0",
    ["M0-04"],
    ["infra"],
    "S",
...
I(
    "M0-12",
    "Decide: loose-folder triage (bips-go-yup 136GB, recovery/, Del, DelEA, DJI Bak) -- source vs junk",
    "M0",
    [],
    ["decision"],
    "S",
...
I(
    "M0-13",
    "Decide: RAW+JPEG pair handling -- keep RAW as archive winner, proxy the embedded JPEG?",
    "M0",
    [],
    ["decision"],
    "S",
...
I(
    "M0-14",
    "Decide: Immich managed-upload tree as the only archive, or also a flat originals tree outside Immich?",
    "M0",
    [],
    ["decision"],
    "S",
...
I(
    "M0-15",
    "Decide: second copy of the archive -- second NAS, Backblaze B2, or rotating USB?",
    "M0",
    [],
    ["decision"],
    "S",
...
I(
    "M0-16",
    "Decide: future ingest path -- keep iPhone to iCloud to Photos loop, or move phone backup to the Immich app?",
    "M0",
    [],
    ["decision"],
    "S",
...
I(
    "M0-17",
    "Decide: export hidden, Recently Deleted, and syndicated (Messages) assets, or skip them?",
    "M0",
    [],
    ["decision"],
    "S",
...
I(
    "M0-18",
    "Decide: timeline and downtime tolerance -- OK with Photos in a mixed proxy/original state for weeks?",
    "M0",
    [],
    ["decision"],
    "S",
...
I(
    "M0-19",
    "Decide: where should Immich ML (faces, CLIP embeddings) run -- NAS only, or a temporary Mac container for the initial backlog?",
    "M0",
    [],
    ["decision"],
    "S",
...
I(
    "M0-20",
    "Decide: fuzzy-match risk tolerance -- auto-drop losers in clean clusters, or review every near-duplicate cluster by eye?",
    "M0",
    [],
    ["decision"],
    "S",
...
I(
    "M1-00",
    "Ingest and dedupe NAS-resident material not in any Photos library, before the live Photos library export starts",
    "M1",
    ["M1-06", "M0-01"],
    ["ingest", "nas", "data-safety"],
    "M",
...
I(
    "M1-01",
    "Source plugin interface: enumerate(), fetch(item)->local path, metadata(item)",
    "M1",
    ["M0-09"],
    ["ingest"],
    "S",
...
I(
    "M1-02",
    "Folder source (recursive, extension allowlist, sidecar discovery XMP/AAE/JSON)",
    "M1",
    ["M1-01"],
    ["ingest"],
    "M",
    "",
    [
        "Sidecars linked to their media by stem",
        "Ignores junk (.DS_Store, thumbs, AppleDouble ._ files)",
    ],
)
I(
    "M1-03",
    "Ingest command: enumerate -> prehash -> full hash -> store -> journal, resumable",
    "M1",
    ["M1-02", "M0-07", "M0-08"],
    ["ingest"],
    "L",
    ...
    [
        "Re-running ingest on unchanged source reports 0 new objects",
        "Kill and resume loses no work",
        "1 TB from USB ingests without manual intervention",
    ],
)
I(
    "M1-04",
    "Apple Photos source via osxphotos (originals + edited + AAE + XMP + JSON metadata)",
    "M1",
    ["M1-01", "M1-00", "M0-17"],
    ["ingest", "mac"],
    "L",
    ...
    [
        "Photos UUID -> object hash mapping stored in journal",
        "Not-downloaded assets are listed in a report, never treated as errors",
        "Optional iCloud fetch batches by date range and respects a free-disk floor",
    ],
)
I(
    "M1-05",
    "Optional iCloud fetch mode: batch download of not-yet-local originals (low priority)",
    "M1",
    ["M1-04"],
    ["ingest", "mac", "optional"],
    "M",
    ...
    [
        "Runs unattended in small batches without exhausting the boot disk",
        "Can be paused and resumed; no other stage depends on it",
    ],
)
I(
    "M1-06",
    "Drop-folder source for the NAS (SMB share, quarantine until stable size, then ingest)",
    "M1",
    ["M1-02"],
    ["ingest", "nas", "data-safety"],
    "M",
    "",
    [
        "File still being written is not ingested",
        "Ingested files moved to processed/ with date subfolders",
    ],
)
I(
    "M1-07",
    "Immich source (optional): pull originals via API for assets not yet in the vault",
    "M1",
    ["M1-01"],
    ["ingest", "immich", "optional"],
    "M",
    "",
...
I(
    "M1-08",
    "Pilot ingest: 5k assets from the loose folders + 1k from the live Photos library",
    "M1",
    ["M1-03", "M1-04"],
    ["pilot"],
    "S",
...
I(
    "M2-01",
    "EXIF/QuickTime extraction with exiftool batch mode",
    "M2",
    ["M0-09"],
    ["fingerprint"],
    "M",
...
I(
    "M2-02",
    "Image perceptual fingerprints (pHash, dHash, blockhash) via vips downscale + sharpness score",
    "M2",
    ["M2-01"],
    ["fingerprint"],
    "M",
    ...
    [
        "Rotated copy is matched (compute hashes for 4 rotations or use rotation-invariant variant)",
        "Resized copy within Hamming <=4 on 256-bit pHash",
    ],
)
I(
    "M2-03",
    "Video fingerprints (frame pHashes at 10/50/90 percent, duration, resolution, bitrate)",
    "M2",
    ["M2-01"],
    ["fingerprint"],
    "M",
...
I(
    "M2-04",
    "Fingerprint command: parallel, resumable, versioned, skips done work",
    "M2",
    ["M2-02", "M2-03"],
    ["fingerprint"],
    "M",
...
I(
    "M2-05",
    "Exact groups (same hash seen from multiple sources)",
    "M2",
    ["M2-04"],
    ["group"],
    "S",
...
I(
    "M2-06",
    "Near groups via BK-tree over pHash with date/dims sanity checks",
    "M2",
    ["M2-04"],
    ["group"],
    "L",
    ...
    [
        "Fixture near-dupes grouped; fixture distinct images not grouped",
        "230k images cluster in <30 min on the Intel i9 Mac",
    ],
)
I(
    "M2-07",
    "Pair groups: RAW+JPEG, Live Photo still+video, edited+original from Photos",
    "M2",
    ["M2-04"],
    ["group"],
    "M",
...
I(
    "M2-08",
    "Sequence groups: BurstUUID, plus time window (<=3 s) with high similarity",
    "M2",
    ["M2-06", "M2-07"],
    ["group"],
    "M",
...
I(
    "M2-09",
    "Group command produces deterministic group ids (hash of sorted member hashes)",
    "M2",
    ["M2-05", "M2-06", "M2-07", "M2-08"],
    ["group"],
    "S",
    "",
...
I(
    "M2-10",
    "Similarity threshold calibration on a labelled sample of 500 clusters",
    "M2",
    ["M2-09", "M1-08", "M0-20"],
    ["group", "research"],
    "M",
    "",
    [
        "Precision >=0.99 on near groups at chosen threshold",
        "Thresholds written into config defaults with rationale",
    ],
)
...
I(
    "M3-01",
    "Winner rule for near groups (pixels > RAW>HEIC>JPEG > size > metadata > earliest)",
    "M3",
    ["M2-09", "M0-13"],
    ["select"],
    "S",
    ...
    [
        ...
        "Edited render never beats its original unless original missing",
    ],
)
I(
    "M3-02",
    "Representative rule for sequences (winner of sharpest frame; faces-open heuristic optional)",
    "M3",
    ["M3-01"],
    ["select"],
    "M",
...
I(
    "M3-03",
    "Decision model: human decisions override rules; effective_state view",
    "M3",
    ["M3-01", "M0-08"],
    ["select"],
    "M",
...
I(
    "M3-04",
    "Image proxy generator (vips): configurable long edge and HEIC/JPEG quality; two profiles: review (720px) and publish (1920px)",
    "M3",
    ["M2-01"],
    ["proxy"],
    "M",
    "",
    [
        "Derived cache keyed by (hash, profile version)",
        "10k images/hour on the Intel i9 Mac",
        "Orientation and ICC preserved",
    ],
)
I(
    "M3-05",
    "Metadata copy into proxies (exiftool -tagsFromFile, all EXIF/GPS/XMP, ContentIdentifier)",
    "M3",
    ["M3-04"],
    ["proxy"],
    "S",
...
I(
    "M3-06",
    "Video proxy generator (ffmpeg): 1080p HEVC, VideoToolbox on Mac, libx265 fallback; .mov; metadata preserved",
    "M3",
    ["M2-01"],
    ["proxy"],
    "M",
...
I(
    "M3-07",
    "Live Photo proxy pairing (still + video share ContentIdentifier after proxying)",
    "M3",
    ["M3-05", "M3-06"],
    ["proxy"],
    "S",
...
I(
    "M3-08",
    "Proxy command: parallel, resumable, only for objects that need it (winners/reps)",
    "M3",
    ["M3-04", "M3-06", "M3-03"],
    ["proxy"],
    "S",
...
I(
    "M3-09",
    "Publish-set calculation and size report (estimate iCloud footprint)",
    "M3",
    ["M3-03", "M3-08"],
    ["select"],
    "S",
...
I(
    "M4-01",
    "Review server (FastAPI + htmx) reading catalog and proxies, local only",
    "M4",
    ["M3-09"],
    ["ui"],
    "M",
...
I(
    "M4-02",
    "Timeline view: infinite scroll by day using review proxies, fast on 230k items",
    "M4",
    ["M4-01"],
    ["ui"],
    "L",
...
I(
    "M4-03",
    "Sequence strip view: burst/near members side by side, sharpness badges, choose representative",
    "M4",
    ["M4-02"],
    ["ui"],
    "M",
...
I(
    "M4-04",
    "Keep/trim actions with keyboard shortcuts, batch select, undo",
    "M4",
    ["M4-03", "M3-03"],
    ["ui"],
    "M",
...
I(
    "M4-05",
    "Filters: year, source, group type, undecided-only, sequences with >N members",
    "M4",
    ["M4-02"],
    ["ui"],
    "S",
...
I(
    "M4-06",
    "Compare view for near-group winners (side-by-side with zoom, dims, size, source)",
    "M4",
    ["M4-03"],
    ["ui"],
    "S",
...
I(
    "M4-07",
    "Contact-sheet HTML export as a no-server fallback for review",
    "M4",
    ["M3-09"],
    ["ui", "optional"],
    "S",
...
I(
    "M5-01",
    "Publish target interface and Photos target via osxphotos import (album, keywords, title, description, location, favorite)",
    "M5",
    ["M3-09"],
    ["publish", "mac"],
    "L",
...
I(
    "M5-02",
    "Verify step: confirm proxy present in Photos by UUID and metadata matches",
    "M5",
    ["M5-01"],
    ["publish", "mac"],
    "M",
...
I(
    "M5-03",
    "Retire step: delete original from Photos by UUID, manually triggered per batch, backup-verified precondition, --yes",
    "M5",
    ["M5-02", "M6-04"],
    ["publish", "mac", "destructive", "data-safety"],
    "M",
...
I(
    "M5-04",
    "Shared Library policy and contributor detection (track osxphotos issue 860)",
    "M5",
    ["M0-01", "M1-04"],
    ["publish", "risk"],
    "M",
...
I(
    "M5-05",
    "Album and favorites round-trip mapping (Photos albums -> proxies in same albums)",
    "M5",
    ["M5-01"],
    ["publish"],
    "M",
    "",
    ["Albums recreated with same names; favorites set"],
)
I(
    "M5-06",
    "Pilot: 500 assets round trip into a scratch Photos library, then 500 into the real one",
    "M5",
    ["M5-02", "M5-05", "M3-07"],
    ["pilot", "mac"],
    "M",
    "",
    [
        "Checklist: dates, GPS, albums, keywords, Live Photos, HEIC rendering, faces re-detected",
        "Sign-off recorded in docs/decisions.md",
    ],
)
I(
    "M5-07",
    "Immich publish target via immich-go (winners, albums, XMP) for viewing",
    "M5",
    ["M3-09"],
    ["publish", "immich", "optional"],
    "M",
    "",
    ["Winners' checksums found via Immich API after upload"],
)
I(
    "M5-08",
    "Folder publish target (plain export of winners by yyyy/MM) for non-Immich users",
    "M5",
    ["M3-09"],
    ["publish", "optional"],
    "S",
...
I(
    "M6-01",
    "Backup target interface: rsync-to-disk and restic (B2/second NAS)",
    "M6",
    ["M0-07", "M0-08", "M0-15"],
    ["backup", "data-safety"],
    "M",
    "",
    ["Targets configurable; archive and journal both included"],
)
I(
    "M6-02",
    "Backup command: incremental, then verify by rehashing a sample and all new objects",
    "M6",
    ["M6-01"],
    ["backup", "data-safety"],
    "M",
    "",
    ["Journal event backup_verified per object per target"],
)
I(
    "M6-03",
    "Scrub command: periodic full rehash of archive and backups, report drift",
    "M6",
    ["M6-02"],
    ["backup", "data-safety"],
    "S",
    "",
    ["Monthly scrub of 8 TB completes; report emailed/logged"],
)
I(
    "M6-04",
    "Restore drill: restore 1k random objects from each target and verify",
    "M6",
    ["M6-02"],
    ["backup", "data-safety"],
    "S",
    "",
    ["Documented runbook; drill passes"],
)
I(
    "M6-05",
    "Offline drive rotation procedure (two USB drives alternating, one off-site)",
    "M6",
    ["M6-02", "M0-03"],
    ["backup", "ops", "data-safety"],
    "S",
    "",
    ["Runbook in docs/backups.md"],
)

# ---------------- M7 Automation ----------------
I(
    "M7-01",
    "`vault sync` orchestrator: ingest -> fingerprint -> group -> select -> proxy -> publish(import) -> backup, with lock file",
    "M7",
    ["M1-04", "M2-09", "M3-08", "M5-01", "M6-02"],
    ["automation"],
    "M",
    ...
    [
        "Single command idempotent; concurrent run refused",
        "Retire (M5-03) is unreachable from this command, enforced by a runtime check not just omission",
    ],
)
I(
    "M7-02",
    "Intel Mac launchd job for nightly sync (ingest through publish/import only, never retire) with disk-space and power preconditions",
    "M7",
    ["M7-01"],
    ["automation", "mac"],
    "S",
...
I(
    "M7-03",
    "NAS container schedule: hourly drop-folder ingest, nightly backup, monthly scrub",
    "M7",
    ["M0-11", "M1-06", "M6-03"],
    ["automation", "nas"],
    "S",
...
I(
    "M7-04",
    "Status, health and notifications (CLI status, JSON report, ntfy/email on failure)",
    "M7",
    ["M7-01"],
    ["automation"],
    "S",
...
I(
    "M7-05",
    "Family upload path: drop folder via SMB/PhotoSync, or Immich mobile -> Immich source",
    "M7",
    ["M1-06", "M1-07", "M0-16"],
    ["automation", "family"],
    "S",
    "",
    ["Documented instructions for family members"],
)
I(
    "M7-06",
    "Catalog concurrency: Mac and NAS both writing (journal per writer, catalog rebuild on conflict)",
    "M7",
    ["M0-09", "M7-03"],
    ["automation", "core"],
    "M",
    "",
    ["Simulated concurrent writes converge after rebuild"],
)

# ---------------- M8 Migration ----------------
I(
    "M8-01",
    "Ingest the March-2025 backup Photos library (bulk originals from USB)",
    "M8",
    ["M1-04", "M1-08", "M0-03"],
    ["migration"],
    "L",
    "",
    ["All assets ingested or listed as unreadable"],
)
I(
    "M8-02",
    "Ingest loose folders (PhotosBackup, MP600, BergePhotos, NAS FastTransfer)",
    "M8",
    ["M1-03", "M0-01", "M0-12"],
    ["migration"],
    "M",
...
I(
    "M8-03",
    "Ingest the live Photos library: locally present originals only",
    "M8",
    ["M8-01"],
    ["migration", "mac"],
    "M",
    "",
    ["Everything downloaded on the Mac is archived; not-downloaded assets listed in a report"],
)
I(
    "M8-09",
    "Optional last step: fetch remaining iCloud-only originals in background batches",
    "M8",
    ["M8-05", "M1-05"],
    ["migration", "mac", "optional"],
    "XL",
...
I(
    "M8-04",
    "Full fingerprint + group + select on the whole vault; publish size report",
    "M8",
    ["M8-02", "M8-03", "M2-10", "M3-09"],
    ["migration"],
    "L",
    "",
    [],
...
I(
    "M8-05",
    "First full backup to two targets and verify before any retire",
    "M8",
    ["M8-04", "M6-04", "M6-05"],
    ["migration", "backup", "data-safety"],
    "L",
    "",
    [],
)
I(
    "M8-06",
    "Publish proxies to Photos year by year, verify, retire originals, monitor iCloud usage",
    "M8",
    ["M8-05", "M5-06", "M5-04", "M0-18"],
    ["migration", "destructive", "data-safety"],
    "XL",
    "",
    [
        "iCloud usage drops as expected per year",
        "Zero assets in Photos without a matching archive object",
        "Each year's retire batch is manually triggered and reviewed -- never automated (M5-03)",
    ],
)
I(
    "M8-07",
    "Sequence trimming passes in the review UI (bursts first, then near groups)",
    "M8",
    ["M8-04", "M4-04"],
    ["migration", "ui"],
    "L",
...
I(
    "M8-08",
    "Decommission old projects and staging; keep one archived copy of decisions",
    "M8",
    ["M8-06"],
    ["migration", "cleanup"],
    "S",
...
I(
    "M9-01",
    "Docs: README, quickstart, architecture, safety model, config reference",
    "M9",
    ["M7-01"],
    ["docs"],
    "M",
...
I(
    "M9-02",
    "Remove all personal paths/defaults; example configs for Mac-only, NAS-only, both",
    "M9",
    ["M9-01"],
    ["docs"],
    "S",
...
I(
    "M9-03",
    "License, contributing guide, issue templates, release workflow (uv build, Docker publish)",
    "M9",
    ["M9-02"],
    ["docs", "infra"],
    "S",
...
I(
    "M9-04",
    "Immich integration documented as optional plugin; Google Takeout source",
    "M9",
    ["M5-07", "M1-07"],
    ["docs", "optional"],
    "M",
...
out = {
    "meta": {
        "project": "photo vault (working name)",
        "generated": str(datetime.date.today()),
        "generator": "scripts/gen_issues.py",
        ...
        "milestones": {
            "M0": "Foundations",
            "M1": "Ingest",
            "M2": "Fingerprint and group",
            "M3": "Select and proxy",
            "M4": "Review UI",
            "M5": "Publish to Photos",
            "M6": "Backup and verify",
            "M7": "Automation",
            "M8": "Migration",
            "M9": "Open source",
        },
...
for i in ISSUES:
    by_ms[i["milestone"]] += EST_DAYS[i["estimate"]]
